...
# Character sprite colors — suited figure with gray hair and sunglasses
COL_HAIR = 13       # gray
COL_HAIR_LIGHT = 7  # white highlights
COL_SKIN = 15       # peach
COL_GLASSES = 0     # black
COL_GLASSES_SHINE = 5  # dark blue glint
COL_SUIT = 0        # black
COL_SUIT_FOLD = 5   # dark blue fold lines
COL_SHIRT = 7       # white collar/cuffs
COL_TIE = 5         # dark blue
COL_SHOES = 1       # dark navy
COL_SHADOW = 1      # dark navy
COL_MOUTH = 4       # dark brown
...
def draw_character_to_bank(ox: int, oy: int, facing: Point, walk_bob: int):
    """Draw suited character sprite at offset (ox, oy) on pyxel.images[0]."""
    img = pyxel.images[0]
    sx, sy = ox, oy
    ...
    # Shadow
    _img_rect(img, sx + 9, sy + 29, 14, 3, COL_SHADOW)
    _img_rect(img, sx + 11, sy + 28, 10, 1, COL_SHADOW)

    # Shoes
    if walk_bob:
        _img_rect(img, sx + 9, sy + 27, 6, 2, COL_SHOES)
        _img_rect(img, sx + 17, sy + 26, 6, 2, COL_SHOES)
    else:
        _img_rect(img, sx + 9, sy + 26, 6, 2, COL_SHOES)
        _img_rect(img, sx + 17, sy + 27, 6, 2, COL_SHOES)

    # Legs
    if walk_bob:
        _img_rect(img, sx + 10, sy + 23, 5, 4, COL_SUIT)
        _img_rect(img, sx + 17, sy + 22, 5, 4, COL_SUIT)
    else:
        _img_rect(img, sx + 10, sy + 22, 5, 4, COL_SUIT)
        _img_rect(img, sx + 17, sy + 23, 5, 4, COL_SUIT)

    # Suit jacket body
    _img_rect(img, sx + 8, sy + 14, 16, 8, COL_SUIT)
    _img_rect(img, sx + 9, sy + 11, 14, 3, COL_SUIT)

    match facing:
        case Point(0, 1):  # DOWN — front view
            # Arms + white cuffs
            if walk_bob:
                _img_rect(img, sx + 5, sy + 13, 3, 8, COL_SUIT)
                _img_rect(img, sx + 24, sy + 12, 3, 8, COL_SUIT)
                _img_rect(img, sx + 5, sy + 20, 3, 2, COL_SHIRT)
                _img_rect(img, sx + 24, sy + 19, 3, 2, COL_SHIRT)
            else:
                _img_rect(img, sx + 5, sy + 12, 3, 8, COL_SUIT)
                _img_rect(img, sx + 24, sy + 13, 3, 8, COL_SUIT)
                _img_rect(img, sx + 5, sy + 19, 3, 2, COL_SHIRT)
                _img_rect(img, sx + 24, sy + 20, 3, 2, COL_SHIRT)

            # V-collar — white shirt tapering under the jacket
            _img_rect(img, sx + 10, sy + 11, 5, 1, COL_SHIRT)
            _img_rect(img, sx + 17, sy + 11, 5, 1, COL_SHIRT)
            _img_rect(img, sx + 11, sy + 12, 4, 1, COL_SHIRT)
            _img_rect(img, sx + 17, sy + 12, 4, 1, COL_SHIRT)
            _img_rect(img, sx + 12, sy + 13, 3, 1, COL_SHIRT)
            _img_rect(img, sx + 17, sy + 13, 3, 1, COL_SHIRT)

            # Tie
            _img_rect(img, sx + 15, sy + 11, 2, 8, COL_TIE)

            # Suit fold lines
            _img_line(img, sx + 12, sy + 15, sx + 12, sy + 20, COL_SUIT_FOLD)
            _img_line(img, sx + 19, sy + 15, sx + 19, sy + 20, COL_SUIT_FOLD)

            # Face
            _img_rect(img, sx + 10, sy + 4, 12, 7, COL_SKIN)
            _img_rect(img, sx + 11, sy + 3, 10, 1, COL_SKIN)

            # Mouth
            _img_rect(img, sx + 14, sy + 9, 4, 1, COL_MOUTH)

            # Sunglasses — two lenses with bridge
            _img_rect(img, sx + 9, sy + 5, 6, 3, COL_GLASSES)
            _img_rect(img, sx + 17, sy + 5, 6, 3, COL_GLASSES)
            _img_rect(img, sx + 15, sy + 6, 2, 1, COL_GLASSES)
            img.pset(sx + 10, sy + 5, COL_GLASSES_SHINE)
            img.pset(sx + 18, sy + 5, COL_GLASSES_SHINE)

            # Hair — gray, swept back
            _img_rect(img, sx + 10, sy + 0, 12, 1, COL_HAIR)
            _img_rect(img, sx + 9, sy + 1, 14, 3, COL_HAIR)
            _img_rect(img, sx + 9, sy + 4, 2, 1, COL_HAIR)
            _img_rect(img, sx + 21, sy + 4, 2, 1, COL_HAIR)
            _img_rect(img, sx + 11, sy + 0, 3, 2, COL_HAIR_LIGHT)
            _img_rect(img, sx + 19, sy + 1, 3, 2, COL_HAIR_LIGHT)

        case Point(0, -1):  # UP — back view
            # Arms + cuffs
            if walk_bob:
                _img_rect(img, sx + 5, sy + 13, 3, 8, COL_SUIT)
                _img_rect(img, sx + 24, sy + 12, 3, 8, COL_SUIT)
            ...
            # White collar line at back of neck
            _img_rect(img, sx + 11, sy + 10, 10, 2, COL_SHIRT)

            # Center back seam
            for yo in range(14, 22, 2):
                img.pset(sx + 16, sy + yo, COL_SUIT_FOLD)

            # Hair — fuller from back, swept down toward collar
            _img_rect(img, sx + 10, sy + 0, 12, 1, COL_HAIR)
            _img_rect(img, sx + 9, sy + 1, 14, 5, COL_HAIR)
            _img_rect(img, sx + 10, sy + 6, 12, 3, COL_HAIR)
            _img_rect(img, sx + 11, sy + 9, 10, 1, COL_HAIR)
            _img_rect(img, sx + 11, sy + 0, 3, 2, COL_HAIR_LIGHT)
            _img_rect(img, sx + 14, sy + 1, 4, 1, COL_HAIR_LIGHT)
            _img_rect(img, sx + 19, sy + 2, 3, 2, COL_HAIR_LIGHT)

        case Point(-1, 0):  # LEFT profile
            # Left arm visible
            if walk_bob:
                _img_rect(img, sx + 6, sy + 13, 3, 8, COL_SUIT)
                _img_rect(img, sx + 6, sy + 20, 3, 2, COL_SHIRT)
            else:
                _img_rect(img, sx + 6, sy + 12, 3, 8, COL_SUIT)
                _img_rect(img, sx + 6, sy + 19, 3, 2, COL_SHIRT)

            # Collar (side view)
            _img_rect(img, sx + 10, sy + 11, 5, 2, COL_SHIRT)

            # Suit fold
            _img_line(img, sx + 14, sy + 14, sx + 14, sy + 20, COL_SUIT_FOLD)

            # Profile face — features on left
            _img_rect(img, sx + 10, sy + 4, 10, 7, COL_SKIN)
            _img_rect(img, sx + 11, sy + 3, 8, 1, COL_SKIN)
            _img_rect(img, sx + 9, sy + 6, 1, 3, COL_SKIN)

            # Mouth
            _img_rect(img, sx + 10, sy + 9, 3, 1, COL_MOUTH)

            # Sunglasses (profile — single lens)
            _img_rect(img, sx + 9, sy + 5, 8, 3, COL_GLASSES)
            img.pset(sx + 10, sy + 5, COL_GLASSES_SHINE)

            # Hair — swept back to the right
            _img_rect(img, sx + 12, sy + 0, 10, 1, COL_HAIR)
            _img_rect(img, sx + 11, sy + 1, 12, 4, COL_HAIR)
            _img_rect(img, sx + 14, sy + 5, 8, 2, COL_HAIR)
            _img_rect(img, sx + 15, sy + 7, 6, 3, COL_HAIR)
            _img_rect(img, sx + 18, sy + 0, 3, 2, COL_HAIR_LIGHT)

        case Point(1, 0):  # RIGHT profile (mirror of left)
            # Right arm visible
            if walk_bob:
                _img_rect(img, sx + 23, sy + 12, 3, 8, COL_SUIT)
                _img_rect(img, sx + 23, sy + 19, 3, 2, COL_SHIRT)
            else:
                _img_rect(img, sx + 23, sy + 13, 3, 8, COL_SUIT)
                _img_rect(img, sx + 23, sy + 20, 3, 2, COL_SHIRT)

            # Collar
            _img_rect(img, sx + 17, sy + 11, 5, 2, COL_SHIRT)

            # Suit fold
            _img_line(img, sx + 17, sy + 14, sx + 17, sy + 20, COL_SUIT_FOLD)

            # Profile face — features on right
            _img_rect(img, sx + 12, sy + 4, 10, 7, COL_SKIN)
            _img_rect(img, sx + 13, sy + 3, 8, 1, COL_SKIN)
            _img_rect(img, sx + 22, sy + 6, 1, 3, COL_SKIN)

            # Mouth
            _img_rect(img, sx + 19, sy + 9, 3, 1, COL_MOUTH)

            # Sunglasses
            _img_rect(img, sx + 15, sy + 5, 8, 3, COL_GLASSES)
            img.pset(sx + 21, sy + 5, COL_GLASSES_SHINE)

            # Hair — swept back to the left
            _img_rect(img, sx + 10, sy + 0, 10, 1, COL_HAIR)
            _img_rect(img, sx + 9, sy + 1, 12, 4, COL_HAIR)
            _img_rect(img, sx + 10, sy + 5, 8, 2, COL_HAIR)
            _img_rect(img, sx + 11, sy + 7, 6, 3, COL_HAIR)
            _img_rect(img, sx + 11, sy + 0, 3, 2, COL_HAIR_LIGHT)
